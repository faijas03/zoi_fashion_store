...
exports.blacklistjwt = async (req,res,next) =>{
    try {
        ...
        const user = await registerschema.findById(req.user.userId).select("username -_id")
        const username = user.username
    console.log(username);
    if(!token) return res.status(404).json({error:"NO valid token"})  
    blacklist.add(token);
    res.status(200).json({message:`${username} logout succesfully,token moved to blacklist`})
