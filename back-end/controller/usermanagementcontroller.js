...
exports.getuserdetails = async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(404).json({ error: "error to fetch user by id" });
...
exports.deleteuser = async (req, res) => {
  try {
    ...
    await addressschema.findOneAndDelete({ userId: req.params.id });
    if (!user) return res.status(404).json({ error: "unable to find user" });
    res
      .status(200)
      .json({ message: `${user.username} account deleted succesfully` });
  } catch (error) {
