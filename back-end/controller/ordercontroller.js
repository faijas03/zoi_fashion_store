...
exports.placeorder = async (req, res) => {
  ...
  try {
    ...
    if (!address)
      return res.status(404).json({ error: "your forget to add address " });
...
exports.getorder = async (req, res) => {
  ...
  try {
    ...
    if (!order)
      return res
        .status(404)
        .json({ error: "there is no order in your order history" });
...
exports.cancelorder = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `order cancelled with orderId: ${orderId}` });
  } catch (error) {
    res.status(400).json({ error: "server on maintenance" });
  }
};
...
exports.getorderbyadmin = async (req, res) => {
  ...
  try {
    const order = await orderschema.findById(orderId);
    if (!order)
      return res.status(404).json({ error: "there is no order in this id" });
    const address = await addressschema.findById(order.userAddress);
...
exports.updateorderbyadmin = async (req, res) => {
  try {
    ...
    if (!updateorder)
      return res.status(404).json({ error: "unable to find order to update" });
