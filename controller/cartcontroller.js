...
exports.getcart = async(req,res) =>{
    const userId = req.user.userId;
    try {
        const username = await registerschema.findOne({_id:userId},{username:1,_id:0})
        console.log(username);
        if(!username) return res.status(404).json({error:"error to load user"})
        const usercart = await cartschema.findOne({user:userId})
        if(!usercart) return res.status(404).json({error:"error to fetch user cart"})
            res.status(200).json({message:`${username} cart is`,
            usercart
            })
    } catch (error) {
...
exports.removefromcart = async (req,res) =>{
    ...
    try {
        ...
        if(!productname) return res.status(404).json({error:"error to find product"})
        const product = await cartschema.findOne({ user: userId, "items.itemid": productId})
        if(!product) return res.status(400).json({error:"no product in cart"})
        const removeproduct  = await cartschema.updateOne({user:userId},{$pull:{items:{itemid:productId}}}) 
        if(!removeproduct) return res.status(404).json({error:"error to laod data"})
            const cart = await cartschema.findOne({user:userId})
        cart.bill = cart.items.reduce((acc,pro)=> {
            return acc+pro.quantity*pro.price            
        },0)
        await cartschema.findOneAndUpdate({user:userId},{$set:{bill:cart.bill}},{new:true})
        if(removeproduct.modifiedCount === 1) 
             res.status(200).json({message:`${productname.productname} removed from cart`})
    } catch (error) {
...
exports.updatequantity = async (req,res)=>{
    ...
    try {
        ...
        const updatecart = await cartschema.findOneAndUpdate({user:userId,"items.itemid":productid},{$set:{"items.$.quantity":req.body.quantity}}, {new:true})
