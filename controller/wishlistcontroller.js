...
exports.addwishlist = async (req,res) =>{
    ...
    try {
        ...
        if(wishlist){
            const newproduct = await wishlistschema.findOne({"product.productId":productId})
            if(newproduct) return res.status(404).json({error:"product already added to wishlist"})
...
exports.getwishlist = async(req,res) =>{
    ...
    try {
        ...
        if(!userwishlist) return res.status(404).json({error:"error to load wishlist"})
...
exports.deletewishlist = async(req,res) =>{
    ...
    try {
        ...
        if(!userwishlist) return res.status(404).json({error:"user have no wishlist"})
...
exports.removeproductfromwishlist = async(req,res) =>{
    ...
    try {
        const product = await wishlistschema.findOne({ user: userId, "product.productId": productId})
        if(!product) return res.status(400).json({error:"no product in wishlist"})
        const removeproduct  = await wishlistschema.updateOne({user:userId},{$pull:{product:{productId:productId}}}) 
