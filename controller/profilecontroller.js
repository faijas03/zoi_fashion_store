...
exports.profileupdate = async (req, res) => {
  try {
    ...
    const { error } = validation.validateupdateprofile.validate(req.body, {
      abortEarly: false,
    });
    if (error) {
      return res
    ...
    const { name, phone, username, gender, street, city, state, postal } =
      req.body;
    ...
    if (Object.keys(userfields).length > 0) {
      updateuser = await registerschema.findByIdAndUpdate(userId, userfields, {
        new: true,
        runValidators: true,
      });
    }
    if (Object.keys(addressfields).length > 0) {
      await addressschema.findOneAndUpdate({ userId: userId }, addressfields, {
        new: true,
        upsert: true,
        runValidators: true,
      });
