    ...
    try {
        if(code||discount||expiresAt||productId||minOrderAmount){
            const newcoupon = await couponschema({
                code,discount,expiresAt,productId,minOrderAmount
            });
...
exports.updatecoupon = async (req,res) =>{
    ...
    try {
        const updateproduct = await couponschema.findByIdAndUpdate(req.params.id,req.body, {new:true})
        if(!updateproduct) return res.status(404).json({error:"unable to find coupon"})
...
exports.deletecoupon = async (req,res) =>{
    try {
        const deleteproduct = await couponschema.findByIdAndDelete(req.params.id, {new:true})
        if(!deleteproduct) return res.status(404).json({error:"unable to find coupon"})
...
exports.getcoupon = async (req,res) =>{
    try {
        ...
        if(!getcoupon) return res.status(404).json({error:"unable to find coupon"})
